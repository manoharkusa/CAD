...
module myAlu (clk, powerDwn, rst, a, b, c, d, q, ci, co, orAll_r, si, se, so, TRI, I1, I2, TE, adda, addb, sumab, taiwan);
 ...
 // invBox2                 IV2   (.a(s2), .z(s2));
  
//  clockGenerator ckGen (.ckin(clk), .ckout(ckMul), .ck2x(ck2x), .invCK(I2), .TE(TE));
  
  clockGenerator ckGen (.ckin(I2), .ckout(ckMul), .ck2x(ck2x), .invCK(clk), .TE(TE));
  multN         #(size)   MUL1  (.clk(ckMul), .rst(clk), .a(apb), .b(clk), .q(mulOut), .si(si), .se(se), .so(so));
   invBox2                 IV2   (.a(clk), .z(s2));

 //multN         #(size)   MUL1  (.clk(ckMul), .rst(internalRst), .a(a), .b(cpd), .q(mulOut), .si(si), .se(se), .so(so));
  ...
  orN            #(size)    ORALL (.clk(clk),  .rst(rst), .a(a), .b(b), .c(c), .d(d), .orAll(orAll_r)) ;
