...
def run_synthesis_qms_checks(stage_dir: Path, block_name: str, rtl_tag: str, project: str) -> Dict[str, Any]:
    ...
    latest_log_path, latest_log_content = get_latest_log_file(logs_dir)
    
    if latest_log_content:
        # Tool version check
        print(f"[QMS] targeted latest log file: {latest_log_path}")
        qms_results["correct_tool_version"] = check_tool_version(latest_log_content, "syn")
        if qms_results["correct_tool_version"]:
            qms_results["correct_tool_version"].report_path = latest_log_path

        # RTL-related checks
        qms_results["no_rtl_errors"] = check_rtl_errors(latest_log_content)
        if qms_results["no_rtl_errors"]:
            qms_results["no_rtl_errors"].report_path = latest_log_path

        qms_results["no_rtl_warnings"] = check_rtl_warnings(latest_log_content)
        if qms_results["no_rtl_warnings"]:
            qms_results["no_rtl_warnings"].report_path = latest_log_path

        # Linking checks
        qms_results["no_unresolved_refs"] = check_unresolved_references(latest_log_content)
        if qms_results["no_unresolved_refs"]:
            qms_results["no_unresolved_refs"].report_path = latest_log_path

        qms_results["no_pin_mismatches"] = check_pin_mismatches(latest_log_content)
        if qms_results["no_pin_mismatches"]:
            qms_results["no_pin_mismatches"].report_path = latest_log_path

        # Constraint checks
        qms_results["no_constraint_errors"] = check_constraint_errors(latest_log_content)
        if qms_results["no_constraint_errors"]:
            qms_results["no_constraint_errors"].report_path = latest_log_path

        qms_results["no_constraint_warnings"] = check_constraint_warnings(latest_log_content)
        if qms_results["no_constraint_warnings"]:
            qms_results["no_constraint_warnings"].report_path = latest_log_path
        
        # Synthesis process checks
        qms_results["no_synthesis_errors"] = check_synthesis_errors(latest_log_content)
        if qms_results["no_synthesis_errors"]:
            qms_results["no_synthesis_errors"].report_path = latest_log_path

        qms_results["no_synthesis_warnings"] = check_synthesis_warnings(latest_log_content)
        if qms_results["no_synthesis_warnings"]:
            qms_results["no_synthesis_warnings"].report_path = latest_log_path
    else:
    ...
    from qms_utils import  check_clock_gating_coverage,  check_optimal_drive_strength  
    special_results.append(check_clock_gating_coverage(reports_dir))
    special_results.extend(check_optimal_drive_strength(reports_dir))
    ...
    from qms_utils import QOR_CHECK_ID_MAP, CHECK_DESIGN_ID_MAP, CHECK_ID_MAP, SPECIAL_CHECK_ID_MAP

    combined_id_map = {}
    combined_id_map.update(QOR_CHECK_ID_MAP)
    combined_id_map.update(CHECK_DESIGN_ID_MAP)
    combined_id_map.update(CHECK_ID_MAP)
    combined_id_map.update(SPECIAL_CHECK_ID_MAP)
