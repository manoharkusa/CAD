...
SPECIAL_CHECK_ID_MAP = {
    "clock_gating_coverage" : "SYN-SC-001", # 
    "optimal_drive_strength" : "SYN-SC-014" # 
}
# ==============================================================================
...
def get_latest_log_file(logs_dir: Path) -> Tuple[Optional[str], Optional[str]]:
    """
    Find the most recently modified log/txt file.
    Returns: Tuple (file_path_string, file_content_string)
    """
    ...
    # Logic: Sort files by modification time (st_mtime) and pick the last one (latest)
    latest_file = max(files, key=lambda f: f.stat().st_mtime)
    
    try:
        # Resolve gets the full absolute path
        return str(latest_file.resolve()), latest_file.read_text(errors='ignore')
    except Exception as e:
        print(f"[WARN] Could not read latest file {latest_file}: {e}")
        return str(latest_file.resolve()), None
...
def check_tool_version(logs: str, stage: str) -> QMSResult:
    ...
    try:
        ...
        if version_match:
            ...
            formatted_value = f"{tool_name.lower()} {version}"

            # expected value must match returned value
            ...
            if version.startswith(('20.', '21.', '22.', '23.', '24.')):
                ...
                return QMSResult(
                    "correct_tool_version",
                    "WARN",
                    f"{tool_name} version {version} may be outdated",
                    expected=expected_value,
                    value=formatted_value
                )
        else:
        ...
        return QMSResult(
            "correct_tool_version",
            "FAIL",
            f"Error checking tool version: {e}"
        )
...
def check_design_report_issues(reports_dir: Path) -> List[QMSResult]:
    ...
    return results
# [Highlighted Change] - Replace the existing check_clock_gating_coverage function with this:

def check_clock_gating_coverage(reports_dir: Path) -> QMSResult:
    """
    Check clock gating coverage from reports/clock_gating.rpt.
    Pass condition: > 85%
    """
    # 1. Define the specific report file
    report_file = reports_dir / "clock_gating.rpt"
    ...
    try:
        ...
        report_realpath = str(report_file.absolute())
    ...
    if not content:
        return QMSResult(
            "clock_gating_coverage", 
            "SKIP", 
            "clock_gating.rpt not found",
            report_path=report_realpath
        )

   
    pattern =  r'Total\s+Gated\s+Flip-flops\s+\d+\s+(\d+\.?\d*)'
    ...
    if match:
        ...
        if coverage > limit:
            status = "PASS"
            msg = f"Coverage {coverage}% is above limit {limit}%"
        else:
            status = "FAIL"
            msg = f"Coverage {coverage}% is below limit {limit}%"
        ...
        return QMSResult(
            "clock_gating_coverage", 
            "SKIP", 
            "Coverage percentage not found in report",
            report_path=report_realpath
        )
...
def check_optimal_drive_strength(reports_dir: Path) -> List[QMSResult]:
    ...
    report_file = reports_dir / "high_drive_cell_check.rpt"
    report_realpath = str(report_file.resolve())

    if not content:
        results.append(
            QMSResult(
                "optimal_drive_strength",
                "SKIP",
                "high_drive_cell_check.rpt not found"
            )
        )
        return results

    highdrive_strength_cells = ["D18", "D20", "D24", "D32"]

    # PASS case (robust match)
    if "PASS:" in content and "No" in content and "cells found" in content:
        results.append(
            QMSResult(
                "optimal_drive_strength",
                "PASS",
                "No high drive strength cells found in the design",
                value=0,
                expected=0,
                report_path=report_realpath
            )
    ...
    # Detect which drive strengths exist
    present_types = []
    for drv in highdrive_strength_cells:
        if f"{drv} Cells:" in content:
            present_types.append(drv)

    # Count actual instances
    inst_count = len(re.findall(r"^inst:", content, re.MULTILINE))

    # Build message
    if len(present_types) > 1:
        msg = f"Found multiple high drive cell types in the design ({', '.join(present_types)})"
    elif len(present_types) == 1:
        msg = f"Found {present_types[0]} high drive cells in the design"
    else:
        msg = "High drive cells found in the design"

    results.append(
        QMSResult(
            "optimal_drive_strength",
            "FAIL",
            f"{msg} (count={inst_count})",
            details="High drive strength cells present in the design",
            value=inst_count,
            expected=0
        )
    )
