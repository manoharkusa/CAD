...
check_floorplan > $report_dir/check_floorplan.rpt
##Check: any dependendencies on this report
#check_floorplan -report_density > $report_dir/check_floorplan.rpt
check_floorplan -report_density -out_file $report_dir/utilization.rpt
##############################################################################
...
report_constraint  -drv_violation_type max_capacitance > $report_dir/max_cap.rpt
report_constraint  -drv_violation_type max_transition > $report_dir/max_tran.rpt
report_constraint  -drv_violation_type max_fanout > $report_dir/max_fanout.rpt

report_power -leakage -out_file $report_dir/report_power.rpt
##Check: why this report is required at postCTS
check_connectivity -ignore_dangling_wires > $report_dir/check_connectivity.rpt
report_qor -format text -out_file $report_dir/report_qor.rpt


if {[regexp "cts" $stage]} {
write_report $report_dir/cts_max_fanout.rpt [get_db cts_max_fanout]
write_report $report_dir/net_wire_lengths.rpt $content
write_report $report_dir/cts_layer_usage.rpt [get_cts_layer_report]
write_report $report_dir/cts_cell_usage.rpt $context
report_min_pulse_width > $report_dir/mpw.rpt
}
...
report_clock_trees -out_file $report_dir/report_clock_trees.rpt
