...
def run_synthesis_qms_checks(stage_dir: Path, block_name: str, rtl_tag: str, project: str) -> Dict[str, Any]:
    ...
    # NEW LOGIC: Get the specific latest log file and its absolute path
    log_path, latest_log_content = get_latest_log_file(logs_dir)
    
    # Fallback to reading all logs if latest log is not specifically found
    all_logs = latest_log_content if latest_log_content else read_all_logs(logs_dir)
    ...
    # Keys are the check names used in qms_results (before ID mapping).
    # [] = advisory only, ["gold"] = gold-only blocking, etc.
    # =========================================================================
    _SYN_BLOCKING_PHASES = {
        # Always blocking — structural failures (no design output possible)
        "no_rtl_errors":            ["bronze", "silver", "gold"],
        "no_unresolved_refs":       ["bronze", "silver", "gold"],
        "no_constraint_errors":     ["bronze", "silver", "gold"],
        "no_synthesis_errors":      ["bronze", "silver", "gold"],
        "no_unresolved_references": ["bronze", "silver", "gold"],  # check_design
        "SYN-SC-015":               ["bronze", "silver", "gold"],  # netlist
        "SYN-SC-016":               ["bronze", "silver", "gold"],  # SDC
        # Silver+ blocking — important quality, waivable at bronze
        "no_pin_mismatches":        ["silver", "gold"],
        "no_constraint_warnings":   ["silver", "gold"],
        # Gold-only blocking — timing/quality targets
        "no_rtl_warnings":          ["gold"],
        "no_synthesis_warnings":    ["gold"],
        ...
        # Advisory only — informational metrics
        "max_logic_levels":         [],
        "instance_count_stable":    [],
        "clock_gating_insertion":   [],
        "reports_complete":         [],
        "proper_elaboration":       [],
        "synthesis_mapping_ok":     [],
        "synthesis_optimization_ok": [],
    }
    for check_name, result in qms_results.items():
        result.blocking_phases = _SYN_BLOCKING_PHASES.get(
